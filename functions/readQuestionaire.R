...
getQuestionaireDataByID=function(verbose, folder, preText,firstQuestion, idLength) {
...
getQuestionaireDataByDate=function(verbose, folder, preText,firstQuestion) {
  #get files in folder (Questionaire Data)
  fileNames=getFileNames(folder,paste(preText,".*Question.*",sep=""),"txt")
  if (verbose>2) {print(fileNames)}
  #get number of questions from first file
...
#cleanData
cleanData=function(dat,toNums,cleanWhiteSpaces) {
  dat$Geschlecht=substr(toChar(dat$Geschlecht),1,1)
  for(toNum in toNums)
  {
