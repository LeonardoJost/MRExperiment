...
#split block*group*deg
mTime5=lmer(reactionTime~endTime*block*group+deg*block+deg*group+deg*endTime+deg*correctSide+MRexperience+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
mTime5.summary=modelSummary(mTime5,0)
#split deg*group
mTime6=lmer(reactionTime~endTime*block*group+deg*block+deg*endTime+deg*correctSide+MRexperience+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
mTime6.summary=modelSummary(mTime6,0)
#split deg*block
mTime7=lmer(reactionTime~endTime*block*group+deg*endTime+deg*correctSide+MRexperience+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
mTime7.summary=modelSummary(mTime7,0)

#nonsignificant effects
#deg*group*time
mTimeDegXGroupXTime=lmer(reactionTime~deg*endTime*group+
                           endTime*block*group+deg*endTime+deg*correctSide+MRexperience+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
...
#deg*block*group
mTimeDegXBlockXGroup=lmer(reactionTime~deg*group*block+
                           endTime*block*group+deg*endTime+deg*correctSide+MRexperience+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
mTimeDegXBlockXGroup.summary=modelSummary(mTimeDegXBlockXGroup,0)

#main effects of two-way-interactions
#block*group
mTime8=lmer(reactionTime~endTime*block*group+deg*endTime+deg*correctSide+MRexperience-block:group+(deg+endTime|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
anova(mTime7,mTime8)

## separate groups
#control group
dataset.rt=datasetA5b[which(datasetA5b$group=="control"),]
...
#treatment group
dataset.rt=datasetA5b[which(datasetA5b$group=="treatment"),]
